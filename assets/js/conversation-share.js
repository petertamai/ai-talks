...
class ConversationShare {
    ...
    resetConversationId() {
        ...
        // Also reset any cached audio files
        this.audioQueue = [];
        this.currentAudioIndex = 0;
        this.isPlaying = false;
        
        // Hide the play button since we have no audio for the new conversation yet
            ...
            if (data.success) {
                ...
                Swal.fire({
                    ...
                    html: `
                        ...
                        <div class="flex my-4">
                            <input type="text" id="share-link-input" value="${shareUrl}" 
...
collectConversationData() {
    ...
    chatElements.forEach((element, index) => {
        ...
        // Create message object with correct role
        const message = {
            id: index,
            role: isSystem ? 'system' : (isHuman ? 'human' : 'assistant'), // Set correct role
...
document.addEventListener('DOMContentLoaded', function() {
    // Force add the share button to the conversation header
    setTimeout(function() {
    ...
    }, 1000); // Give the page enough time to load
});